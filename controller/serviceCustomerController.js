...
const getParentDropdown = catchAsyncError(async (req, res, next) => {
  ...
  // const data = await serviceCustomerModel.find().lean();
  const data = await serviceCustomerModel
    .find({}, "name serviceCustomer_id")
    .lean();

  console.log("serviceCustomer list----------------", data);
...
async function getAllLeafNodes(data) {
  ...
  let parents = await serviceCustomerModel.find({
    name: { $ne: "Primary" },
...
const getLeafserviceCustomerList = catchAsyncError(async (req, res, next) => {
  console.log("getLeafserviceCustomerList");
  const leafNodes2 = await serviceCustomerModel.aggregate([
    ...
    { $match: { isLeaf: true } },
    { $project: { _id: 1, name: 1, parent_name: 1, serviceCustomer_id: 1 } },
  ]);
...
const getserviceCustomerWiseFilterList = catchAsyncError(
  async (req, res, next) => {
    ...
    const stringIds = [];
    leafNodes.map((res) => {
      stringIds.push(res.serviceCustomer_id.toString());
    });
...
module.exports = {
  getParentDropdown,
  getLeafserviceCustomerList,
  getDataWithPagination,
  getById,
  createData,
  updateData,
  deleteData,
  getserviceCustomerWiseFilterList,
};
