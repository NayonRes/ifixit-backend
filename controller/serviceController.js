...
const getParentDropdown = catchAsyncError(async (req, res, next) => {
  ...
  // const data = await serviceModel.find().lean();
  const data = await serviceModel.find({}, "name service_id").lean();
...
async function getAllLeafNodes(data) {
  ...
  let parents = await serviceModel.find({
    name: { $ne: "Primary" },
...
const getLeafserviceList = catchAsyncError(async (req, res, next) => {
  console.log("getLeafserviceList");
  const leafNodes2 = await serviceModel.aggregate([
    ...
    { $match: { isLeaf: true } },
    { $project: { _id: 1, name: 1, parent_name: 1, service_id: 1 } },
  ]);
...
const getserviceWiseFilterList = catchAsyncError(async (req, res, next) => {
  ...
  const stringIds = [];
  leafNodes.map((res) => {
    stringIds.push(res.service_id.toString());
  });
