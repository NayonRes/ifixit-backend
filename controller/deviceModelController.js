const modelOfDevice = require("../db/models/deviceModel");
...
const index = catchAsyncError(async (req, res, next) => {
    ...
    let totalData = await modelOfDevice.countDocuments(filter);
    console.log("totalData=================================", totalData);
    const data = await modelOfDevice.find(filter).skip(startIndex).limit(limit);
...
const show = catchAsyncError(async (req, res, next) => {
    let data = await modelOfDevice.findById(req.params.id);
    if (!data) {
...
const store = catchAsyncError(async (req, res, next) => {
    ...
    const data = await modelOfDevice.create(newData);
...
const update = catchAsyncError(async (req, res, next) => {
    ...
    data = await modelOfDevice.findByIdAndUpdate(req.params.id, newData, {
        new: true,
        runValidators: true,
        useFindAndModified: false,
    });

    const childrenParentUpdate = await modelOfDevice.updateMany(
...
const dropdown = catchAsyncError(async (req, res, next) => {
    try {
        ...
        const data = await modelOfDevice.find({ parent_id: null}, "name _id").lean();
        console.log(data)
