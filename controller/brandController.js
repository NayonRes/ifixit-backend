...
const index = catchAsyncError(async (req, res, next) => {
    ...
    let totalData = await brandModel.countDocuments(filter);
    console.log("totalData=================================", totalData);
    const data = await brandModel.find(filter).skip(startIndex).limit(limit);
...
const dropdown = catchAsyncError(async (req, res, next) => {
    try {
        ...
        const data = await brandModel.find({ parent_id: null}, "name _id").lean();
        console.log(data)
